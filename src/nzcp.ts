...
function decodeCBOR(stream: Stream) {
  ...
  function decode(stream: Stream): Data {
    ...
    if (type === 0) { // positive int
      ...
      const d = new Array(decodeUint(stream, v)).fill(undefined).map(_ => decode(stream));
      return d
    } else if (type === 5) { // object
      const d: Record<string, Data> = Object.fromEntries(new Array(decodeUint(stream, v)).fill(undefined).map(_ => [decode(stream), decode(stream)]));
      return d
    }
    // return null
    throw VerificationError.invalidData();
